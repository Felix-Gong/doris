...
# FE gRPC protoc plugin (RISC-V). grpc-java ships a linux-riscv64 classifier,
# so download it into the installed bin dir where fe/fe-grpc/pom.xml points
# (persistent across reboots, unlike /tmp).
build_fe_grpc_java_riscv() {
    if [[ "$(uname -m)" != "riscv64" ]]; then
        return
    fi
    if [[ -x "${TP_INSTALL_DIR}/bin/protoc-gen-grpc-java" ]]; then
        echo "protoc-gen-grpc-java already installed"
        return
    fi
    local grpc_java_version="1.53.0"
    ...
    chmod +x "${dest}"
    echo "protoc-gen-grpc-java installed: ${dest}"
}
